...
public class Main {
	...
	public static void main(String[] args)  throws IOException{
		...
		// A verbosidade define se o Motor deve exibir mensagens para cada Jogada feita pelos Jogadores (0=não mostrar mensagens, 1=mostrar mensagens simples, 2=mostrar mensagens simples e detalhadas da situacao do jogo).
		int verbosidade = 2;
		...
		multiplasPartidas(baralho1, baralho2, verbosidade, tempoLimite, saidaArquivo, funcionalidades,500000);
	}
	...
	private static void umaPartida(Baralho baralho1, Baralho baralho2, int verbosidade, int tempoLimite, PrintWriter saidaArquivo, EnumSet<Funcionalidade> funcionalidades){
		...
		Motor partida = new MotorRA172358(baralho1, baralho2, Mao1, Mao2, jogA, jogB, verbosidade, tempoLimite, saidaArquivo, funcionalidades);
	...
	private static void multiplasPartidas(Baralho baralho1, Baralho baralho2, int verbosidade, int tempoLimite, PrintWriter saidaArquivo, EnumSet<Funcionalidade> funcionalidades, int n){
		...
		for(int p = 0; p < 2; p++){
			for(int w = 0; w < n/2; w++){
				...
				// Inicializa os Jogadores devidamente (com as cartas da mão e o argumento 'primeiro').
				//Jogador jogA = new RUIMJogadorRA172358(mao1clone, moeda);
				//Jogador jogB = new RUIMJogadorRA172358(mao2clone, !moeda);
				Jogador jogA = new JogadorTeste(mao1clone, moeda, funcionalidades);
				Jogador jogB = new JogadorTeste(mao2clone, !moeda, funcionalidades);
				...
				Motor partida;
				if(moeda)
					partida = new MotorRA172358(deck1, deck2, Mao1, Mao2, jogA, jogB, verbosidade, tempoLimite, saidaArquivo, funcionalidades);
				else
					partida = new MotorRA172358(deck2, deck1, Mao2, Mao1, jogB, jogA, verbosidade, tempoLimite, saidaArquivo, funcionalidades);
