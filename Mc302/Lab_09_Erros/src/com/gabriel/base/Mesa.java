...
public class Mesa {
    ...
    public void decPoderHeroi(int dec, char heroi) {
        if(heroi == 'P') {
    ...
    public Carta sacarCarta(char autor) {
        ...
        if(autor == 'P' && lacaiosP.size() > 1) {
            return lacaiosP.remove(carta.randInt(gerador, 0, lacaiosP.size()-1) );
        } else if(lacaiosS.size() > 1) {
            return lacaiosS.remove(carta.randInt(gerador, 0, lacaiosS.size()-1));
        }
        return null;
    }
}
