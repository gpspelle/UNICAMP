/* Nome: Gabriel Pellegrino da Silva - EC 016 - Turma: S - RA: 172358
         O programa a seguir coleta as informacoes preenchidas em um formulario, monta a partir desta, um grafo composto pelos vértices, 
	 ...
	 que visavam calcular a sintonia entre a pessoa mais popular do grupo e as pessoas pelas quais o mais popular se interessaria. 
 */
	...
	for(i=0; i<N; i++) 
	{
		...
		{ //condicoes: sexo da pessoa i igual à preferencia do mais popular, a pessoa i nao ser a pessoa mais popular, 
		  //e a media entre a pessoa i e a pessoa mais popular ser maior do que 5, mas tambem realizar o Soundex para o mais popular		
			f=0; //variavel para armazenar quantas vezes a execucao abaixo foi realizada
			aux=tolower((ponteiroStrP+i)->nome[0]); //variavel para armazenar a primeira letra do nome da pessoa
			*(ponteiroSoundex+4*i) = (ponteiroStrP+i)->nome[0]; //colocando no Soundex a primeira letra do nome da pessoa
			...
			{ //Coletando as primeiras tres consoantes do nome e criando o nome codificado das pessoas
				...
				{ 
					case 'j': 
					case 'g': if(aux=='g' || aux=='j') { break;} f++; *(ponteiroSoundex+4*i+f) = 1; aux = 'g';  break;
					case 's':
					case 'z':
					case 'x': if(aux=='x' || aux=='s' || aux=='z') { break;} f++; *(ponteiroSoundex+4*i+f) = 2; aux = 'x';  break;
					case 'c':
					case 'k':
					case 'q': if(aux=='q' || aux=='c' || aux=='k') { break;} f++; *(ponteiroSoundex+4*i+f)=3; aux = 'q';  break;
					case 'b':
					case 'p': if(aux=='p' || aux=='b') { break;} f++; *(ponteiroSoundex+4*i+f)=4; aux = 'p';  break;
					case 'm':
					case 'n': if(aux=='n' || aux=='m') { break;} f++; *(ponteiroSoundex+4*i+f)=5; aux = 'n';  break;
					case 'd':
					case 't': if(aux=='t' || aux=='d') { break;} f++; *(ponteiroSoundex+4*i+f)=6; aux = 't';  break;
					case 'f': 
					case 'v': if(aux=='v' || aux=='f') { break;}  f++; *(ponteiroSoundex+4*i+f)=7; aux = 'v'; break;
					case 'l': if(aux=='l') { break;} f++; *(ponteiroSoundex+4*i+f)=8; aux = 'l'; break;
					case 'r': if(aux=='r') { break;} f++; *(ponteiroSoundex+4*i+f)=9; aux = 'r'; break;
					case ' ': para=0; break;
					default: break; 
				}	
			}	
			//adicionando a quantidade de zeros necessário, dependendo de quantas vezes o bloco acima foi executado					
			if(f==0) { *(ponteiroSoundex+4*i +1)= 0; *(ponteiroSoundex+4*i+2) = 0; *(ponteiroSoundex+4*i+3) = 0; } 
			else if(f==1) { *(ponteiroSoundex+4*i+2) = 0; *(ponteiroSoundex+4*i+3) = 0; } 
			else if(f==2) { *(ponteiroSoundex+4*i+3) = 0; }  
		}	
	}
}
