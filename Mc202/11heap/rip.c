...
void remover(int *vetor, int **heap, int N) { 
	...
	if(pos<0) {
		...
		pos--;
		while(2*aux<pos) {
			if(heap[aux][0]>heap[2*aux+1][0] && heap[2*aux+1][0] < heap[2*aux+2][0]) { 
			...
			else if(heap[aux][0]>heap[2*aux+1][0] && (2*aux+2)>pos) {
				goto OTHER;			
			}
			else if(heap[aux][0]>heap[2*aux+2][0] && (2*aux+2)<=pos) {
...
void diminuir(int *vetor, int **heap, int N) { 
	...
	printf("Aux: %d e %d\n", aux, (2*aux)+1);
	aux = (2*aux)+1;
	if(aux <= pos && heap[aux][0]>heap[aux+1][0]) {
		guarda = heap[aux][1];
		guarda2 = vetor[guarda];
		vetor[guarda] = vetor[heap[aux+1][1]];
		vetor[heap[aux+1][1]] = guarda2;

		guarda = heap[aux][0];
		heap[aux][0] = heap[aux+1][0];
		heap[aux+1][0] = guarda;
